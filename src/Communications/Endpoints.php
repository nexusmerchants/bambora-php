...
class Endpoints
{
    ...
    public function __construct($platform, $version)
    {
        ...
        $this->tokenizationURL = ($platform == 'www' ?
            self::BASE_URL_BEANSTREAM . '/scripts/tokenization/tokens' : $baseUrl . '/scripts/tokenization/tokens');
    ...
    public function getContinuationsURL($merchant_data)
    {
        return str_replace(
            ['{0}', '{1}', '{2}'],
            [$this->platform, $this->version, $merchant_data],
            $this->continuationsURL
        );
    }
    ...
    public function getPreAuthCompletionsURL($tid)
    {
        ...
        return str_replace(
            ['{0}', '{1}', '{2}'],
            [$this->platform, $this->version, $tid],
            $this->preAuthCompletionsURL
        );
    }
    ...
    public function getProfileURI($pid)
    {
        ...
        return str_replace(
            ['{0}', '{1}', '{2}'],
            [$this->platform, $this->version, $pid],
            $this->profileURI
        );
    }
    ...
    public function getCardURI($pid, $cid)
    {
        ...
        return str_replace(
            ['{0}', '{1}', '{2}', '{3}'],
            [$this->platform, $this->version, $pid, $cid],
            $this->cardURI
        );
    }
